...
@section ('contenido')
	...
	{!!Form::model($prestacion, ['method'=>'PATCH', 'route'=>['profesional.prestacion.update', $prestacion->idprestacion],'files'=>'true'])!!}
	{{Form::token()}}
	<div class="row">
		<div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
			<div class="form-group ">
				<label> Nombre</label>
				<input class='form-control' style="text-transform:uppercase;" onkeyup="aMays(event, this)" onblur="aMays(event, this)" title="Se necesita un nombre" required value="{{$prestacion->nombre}}" type="text" name="nombre" required/>
		...
		<div class="col-lg-2 col-sm-2 col-md-2 col-xs-12">
			...
			<div class="col-lg-12 col-sm-12 col-md-12 col-xs-12">
					<table id="detalles" class="table table-striped table-bordered table-condensed">
						...
						<tfoot>
							@foreach ($insumos as $ins)
							<th><tr class="selected" id="fila'+cont+'"><td><button type="button" class="btn btn-warning" onclick="eliminar('+cont+')" >X</button></td><td><input type="hidden" name="idarticulo[]" value="'+idarticulo+'">{{$ins->nombre}}</td><td><input type="number" name="cantidad[]" value="{{$ins->cantidad}}"></td><td><input type="number" name="precio_venta" value="{{$ins->precio_venta*$ins->cantidad}}"></td><td></td></tr></th>
							@endforeach 
							<th>Total</th>
							<th></th>
